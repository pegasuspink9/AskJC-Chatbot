...
import { searchSchoolProgram } from "../../../../helper/services/schoolProgram.database";
...
export const programQuery = async (
  userId: number,
  message: string,
  ...
  try {
        ...
        switch (action) {
          case "get_program_info": {
            const mappedParameters = {
              ...
              department_name: parameters.department_name,
            };

            console.log("Original Dialogflow parameters:", parameters);
            console.log("Mapped program parameters:", mappedParameters);
            ...
            const dbResult = await searchSchoolProgram(cleanedParameters);
            ...
            try {
              ...
              if (
                dbResult.includes("No programs matched your search criteria.") ||
                dbResult.includes("No programs found in the database.")
              ) {
    ...
    console.error("Error in programQuery:", error);
