  ...
  if (dept.head_name) {
    ...
    if (location.length > 0) {
      response += ` You can find ${
        dept.head_name
      } at the department office located at ${location.join(", ")}.`;
    }
    ...
    if (dept.description) {
      response += ` ${
        dept.head_name
      } oversees ${dept.description.toLowerCase()} programs and activities.`;
    }
...
export function formatDepartmentCareerPath(dept: any): string {
  if (dept.career_path) {
    ...
    if (dept.head_name) {
      ...
      if (location.length > 0) {
        response += ` Visit the department office at ${location.join(
          ", "
...
export function formatDepartmentGeneral(dept: any): string {
  ...
  if (details.length > 0) {
    ...
    if (dept.head_name && (dept.building || dept.floor)) {
      ...
      response += `\n\n💼 **Contact:** Visit ${
        dept.head_name
      } at ${location.join(", ")} for inquiries and guidance.`;
    }
...
export function formatMultipleDepartments(departments: any[]): string {
  if (departments.length <= 3) {
    ...
    const list = departments
      .map((dept) => formatDepartmentGeneral(dept))
      .join("\n\n");
    return header + list;
  } else {
    ...
    const list = departments
      .map((dept, i) => {
        ...
        if (dept.career_path)
          details.push(`Careers: ${dept.career_path.split(",")[0]}...`); // Show first career only for brevity
