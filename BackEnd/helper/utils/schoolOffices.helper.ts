...
export function generateMultipleOfficesResponse(offices: any[], query_type?: string): string {
  if (offices.length <= 3) {
    ...
    const list = offices
      .map((o, i) => `${i + 1}. ${o.office_name}${o.description ? ` - ${o.description}` : ""}${o.location_building ? ` (Building: ${o.location_building})` : ""}${o.location_floor ? ` - Floor: ${o.location_floor}` : ""}${o.operating_hours ? ` - Hours: ${o.operating_hours}` : ""}${o.contact_email ? ` - Email: ${o.contact_email}` : ""}${o.contact_phone ? ` - Phone: ${o.contact_phone}` : ""}${o.fb_page ? ` - FB: ${o.fb_page}` : ""}`)
      .join("\n");
    return header + list;
  }
}
...
export function generateNotFoundOfficeMessage(
  ...
  if (office_name) return `I couldn't find any office office_named ${office_name}.`;
